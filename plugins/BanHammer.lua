...
local function kick_ban_res(extra, success, result)
      ...
      elseif get_cmd == 'banall' then
        send_large_msg(receiver, '> ['..user_id..' ] Banned for all @AFhero Groups/SuperGroups! (Globally banned)')
		banall_user(member_id)
      elseif get_cmd == 'unbanall' then
        send_large_msg(receiver, '> ['..user_id..' ] Unbanned for all @AFhero Groups/SuperGroups! (Unglobally banned)')
    ...
    local chat_id = msg.to.id
      if string.match(matches[2], '^%d+$') then
        if tonumber(matches[2]) == tonumber(our_id) then
        	...
        	return '> ['..user_id..' ] Unbanned for all @AFhero Groups/SuperGroups! (Unglobally banned)'
    else
		local cbres_extra = {
			chat_id = msg.to.id,
			get_cmd = 'unbanall',
